function [stimulus, response, confidence] = simulate_model(mu, sigma_sens, sigma_decision, sigma_meta, criteria, N)

%%%%%%%%%%%%%%%%%%%%%%%%%
% Simulate a hierarchical model of confidence. Confidence is given on the x
% axis activation (appropriate for when conditions are mixed). For a script
% in which confidence is given on the posterior (appropriate for when
% conditions are blocked), use simulate_model2.
%%%%%%%%%%%%%%%%%%%%%%%%%
...
for critNum=1:length(criteria)
    crit_bin = crit_bin + (stimValueMeta > criteria(critNum));
end
